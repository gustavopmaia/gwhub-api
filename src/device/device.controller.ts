...
export class DeviceController {
  ...
  async create(@Body() body: any, @Res() res: FastifyReply) {
    ...
    if (result.type === 'validation') return res.status(400).send(errorMessage('Erro de validação', 3, result.detail))
  ...
  async update(@Param('id') id: string, @Body() body: any, @Res() res: FastifyReply) {
    ...
    if (result.type === 'not_found') return res.status(404).send(errorMessage('Dispositivo não encontrado', 2, 'Erro'))
    if (result.type === 'validation') return res.status(400).send(errorMessage('Erro de validação', 3, result.detail))
