...
export const NotificationController = {
  addUser: async (req: FastifyRequest, res: FastifyReply) => {
    ...
    try {
      ...
      res
        .status(400)
        .send(
          errorMessage("Erro ao cadastrar usuário de notificação", 3, "Erro")
        );
    }
  },

  sendNotification: async (req: FastifyRequest, res: FastifyReply) => {
    const token = req.headers["authorization"];
